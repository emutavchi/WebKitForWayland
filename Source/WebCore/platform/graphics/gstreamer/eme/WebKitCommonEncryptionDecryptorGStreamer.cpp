...
static GstFlowReturn transformInPlace(GstBaseTransform* base, GstBuffer* buffer)
{
    ...
    if (!priv->cdmProxy) {
        ...
        if (!priv->cdmProxy) {
            GST_ELEMENT_ERROR (self, STREAM, FAILED, ("CDMProxy was not retrieved in time"), (NULL));
    ...
    if (!ivSize && !gst_structure_get_uint(protectionMeta->info, "constant_iv_size", &ivSize)) {
        GST_ELEMENT_ERROR (self, STREAM, FAILED, ("No iv_size and failed to get constant_iv_size"), (NULL));
        ASSERT(isCbcs);
        return GST_FLOW_NOT_SUPPORTED;
    }
    ...
    if (subSampleCount) {
        ...
        if (!subSamplesBuffer) {
            GST_ELEMENT_ERROR (self, STREAM, FAILED, ("There is no subsamples buffer, but a positive subsample count"), (NULL));
            return GST_FLOW_NOT_SUPPORTED;
        }
    }
    ...
    if (!value) {
        GST_ELEMENT_ERROR (self, STREAM, FAILED, ("Failed to get key id for buffer"), (NULL));
        return GST_FLOW_NOT_SUPPORTED;
    }
